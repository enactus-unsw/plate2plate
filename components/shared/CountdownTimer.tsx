...
export function CountdownTimer({ expiresAt, size = "sm" }: CountdownTimerProps) {
  ...
  if (!timeLeft) {
    return (
      <span className={`inline-flex items-center gap-1.5 font-dm-mono font-medium text-p2p-text-secondary ${textClass}`}>
  ...
  return (
    <span className={`inline-flex items-center gap-1.5 font-dm-mono font-medium ${colorClass} ${textClass}`}>
      <Clock size={iconSize} className="shrink-0" />
      {formatted}
    </span>
  );
